...
// create a name for a temporary file for pass 1 translation
// use a random number in a string:  /tmp/billxxxx  where xxxx is a random number
        gettimeofday (&randtime, (struct timezone *)0);
        srandom((unsigned int)randtime.tv_usec);
        sprintf(temp_file, "/tmp/bill%d", (unsigned int)random()%10000);

// open temporary file and immediately remove filename from  /tmp  directory
...
***/
          ...
          case 23: if((tok=yylex()) != INTEG){
		   ...
		   str_8(yytext);
                   ...
                   break;
//RSHIFT (1111111100xxmmmm)
          case 24: if((tok=yylex()) != INTEG){ 
...
//MULT   (1111111101mmmmmm)
          case 25:  if((tok=yylex()) != INTEG){ 
           ...
           break; 
//DIV    (1111111110000000)
          case 26:  fprintf(p1,"%d 1111111110000000\n",pc);
                        break;

//HALT   (1111111111000000)
          case 27: fprintf(p1,"%d  1111111111000000\n",pc);
                   break;


          case 28:  str_16(yytext);
		    fprintf(p1,"%d  %s\n", pc, cstr_16);
 /*                   fprintf(stderr,"Misplaced integer is %s\n",yytext);   */
 /*                   exit(25);     */
                    break;

	  case 29:  if (label_pc == pc){	/* for < lbx: lby: >   */
			fprintf(p1,"%d  U0000000000000000    %s\n", pc, yytext);
		        break;
		    }
	  ...
	  case 31:  i=1;
          ...
          case 32:  fprintf(stderr,"Unrecognized token is %s\n",yytext);
                    exit(26);
